<?php
/* $Id: xmlrpc.php,v 1.4 2005/03/24 09:59:31 seichter Exp $*/
/*
...
Note: fixed by Hartmut Seichter (http://technotecture.com) in regard to call-by-reference
      problems. It has been tested with PHP 4.3.8, 4.3.9 and 5.0.2

Note: Hack inserted by klickverbot (klickverbot@gmail.com) to surpress the remaining
      call-by-reference warning.
*/
 ...
 * @param $array array
 * @param $value mixed
 * @desc Prepend a reference to an element to the beginning of
 * an array. Renumbers numeric keys, so $value is always inserted to $array[0]
 */
...
function & XMLRPC_prepare($data, $type = NULL){
   if(is_array($data)){
      ...
      if((array_key_exists( 0, $data ) || ( $num_elements == 0 ) ) && ( $type != 'struct' ) ) { #it's an array
      ...
      if(!$type){
         if(is_int($data)){
            ...
            /* FIX - otherwise uncontrolable encoding */
